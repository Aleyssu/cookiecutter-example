# CISC-804 Modelling Project

Welcome to the major project for CISC-804!
...
`docker build -t cisc804 .`

4. Now that we have the image we can run the image as a container by using the command: `docker run -it -v $(pwd):/PROJECT cisc804 /bin/bash`
...
```
docker build --platform linux/x86_64 -t cisc804 .
```
...
```
docker run -it -v "%cd%":/PROJECT cisc804
```
